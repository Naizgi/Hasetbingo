...
async def admin_start_game(request):
    ...
    try:
        ...
        if game.get('status') != 'card_purchase':
            return web.json_response({
                'success': False,
                'message': f'Game is already in {game.get("status")} status'
            }, status=400)
        
        # Start the game via game_manager
        await game_manager.start_game_play(game_id)
...
async def admin_stop_game(request):
    ...
    try:
        ...
        if game.get('status') == 'winner_display':
            return web.json_response({
                'success': False,
                'message': 'Game already has a winner'
            }, status=400)
